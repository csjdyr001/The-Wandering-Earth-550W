...
mode = 0
# 设置 API 密钥和 API URL
apikey = "sk-cc3db1d6e79eebcbaf7f297b4bca47bbAPIGPT-v2"  #
...
def command(resultText):
    ...
    if '' == resultText or ' ' == resultText or '\n' == resultText:
        ...
        if "重启系统" in result:
            ...
            if "切换在线版" in result:
                print("-服务器连接失败，正在重启北京根服务器…")
                print("\n-正在覆写服务器…")
                for i in tqdm(range(100), desc='覆写进度', colour="red"):
                    print(letters(random.randint(1, 6)))
                    time.sleep(random.randint(1, 3) / 100)
                print("正在重启北京根服务器…")
                total = 100
                progress = tqdm(total=total, position=0, leave=True, desc='重启进度', colour="red")
                for i in range(total):
                    progress.update(1)
                    if i == int(total * 0.97):
                        print("-userName: Tu Hengyu")
                        global apikey
                        apikey_input = input("-passWord（chatGPT密钥，输入“none”使用默认密钥（默认密钥可能无效））:")
                        print("-passWord:" + "*" * len(apikey_input))
                        if apikey_input != "none":
                            apikey = apikey_input
                        # progress.resume()
                    else:
                        if i == total - 1:
